...
{
    public class AdiSpider
    {
        private static IDbConnection _connection;

        #region [Variables/Constants]
        public static string siteUrl = "http://adiglobal.us";
        ...
        public static bool Login()
        {
            while (true)
            {
                lock (LoginLock)
                {
                    ...
                    String Url = Settings.GetValue("ADILoginPage");
                    ...
                    formData["ctl00$PlaceHolderHeader$headerContent_cntrl$ADILoginView$MainLoginView$MainLogin$Password"] = Settings.GetValue("ADIPassword");
                    formData["ctl00$PlaceHolderHeader$headerContent_cntrl$ADILoginView$MainLoginView$MainLogin$UserName"] = Settings.GetValue("ADIUsername");
                    HtmlAgilityPack.HtmlDocument doc = browser.PostRequest(Url, null, formData);
                    LoggedIn = doc.DocumentNode.OuterHtml.Contains("Sign Out");
                    if (LoggedIn)
                    {
        ...
        private static Boolean IsLoggedIn()
        {
            try
            {
                HtmlAgilityPack.HtmlDocument doc = browser.GetWebRequest("https://adiglobal.us/Pages/default.aspx");
                LoggedIn = true;
                return LoggedIn = doc.DocumentNode.OuterHtml.Contains("Sign Out");
            }
            catch
            {
                LoggedIn = false;
        ...
        public static bool Logout()
        {
            ...
            String Url = "https://adiglobal.us/_layouts/signout.aspx";
        ...
        {
            String Url = "http://adiglobal.us/_vti_bin/requests.asmx/SubmitQuery";
        ...
        public static List<AdiCategory> GetAllCategories(String SearchCategory, String ExcludedRefiners, List<AdiSearchCriteria> adiSearchCriteriaList)
        {
            ...
            if (categories.Count > 0)
            {
                for (int index = 0; index < categories.Count; index++)
                {
                    ...
                    categories[index].CategoryUrl = String.Format("http://adiglobal.us/Pages/Results.aspx?c={0}&m=c", categories[index].Value);
        ...
        private static String SaveProductBigImage(AdiProduct p, AdiMode mode)
        {
            ...
            String FileName = String.Format("{0}\\{1}\\{2}", GetImageFolder(mode), CreateProductDirectory(p,mode), ImageFileName);
            String Url = (p.BigImage.StartsWith("http") ? "" : "http:") + p.BigImage;
            //Utility.ApplicationLog(String.Format("Downloading Image from {0} to {1}", Url, FileName));
            browser.DownloadFile(Url, FileName);
        ...
        private static string GetAbsoluteUrl(String Url)
        {
            ...
            if (LoggedIn)
                ...
                absoluteUrl = "http://adiglobal.us" + Url;
            return absoluteUrl;
        }
        #endregion
